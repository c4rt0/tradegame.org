...
def validate_token(token: str) -> dict:
    ...
    token = str(token)
    try:
        data = jwt.decode(token, os.environ['jwt_secret'], algorithms=os.environ['jwt_algo'])
        print(data)
        return data, 
...
def create_token(user_id: str, is_admin: bool):
    """
    creates an token that is used to authenticate the user
    """
    timeout_time = 60 * 60 * 4 # seconds, minutes, hours
    expire_time = time.time() + timeout_time 
    token = jwt.encode(
                {'userid': user_id, "is_admin": is_admin, "exp":int(expire_time), "creationTime": str(time.time())}, os.environ['jwt_secret'], algorithm=os.environ['jwt_algo'])
